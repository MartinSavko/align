...
class collect(object):
    ...
    def __init__(self,
        ...
        if self.Range != None:
            self.nbFrames = int(
                math.ceil((self.Range - self.startAngle) / self.oscillation))
        else:
        ...
        if self.inverse is not None:
            self.template_inv = template.replace(
                '_' + str(self.run) + '_',  '_' + str(int(run) + 1) + '_')
    ...
    def initializeDevices(self):
        # Initialize all the devices that are going to be used during the
        # collect
        from PyTango import DeviceProxy as dp
        if self.test: 
            return
        self.md2 = dp('i11-ma-cx1/ex/md2')
        self.publisher = dp('i11-ma-cx1/ex/md2-publisher')
        self.phase = dp('i11-ma-cx1/ex/md2-phase')
        self.adsc = dp('i11-ma-cx1/dt/adsc')
        self.limaadsc = dp('i11-ma-cx1/dt/limaadsc')
        self.header = dp('i11-ma-cx1/ex/header')
        self.mono1 = dp('i11-ma-c03/op/mono1')
        self.detector_mt_ts = dp('i11-ma-cx1/dt/dtc_ccd.1-mt_ts')
        self.detector_mt_tx = dp('i11-ma-cx1/dt/dtc_ccd.1-mt_tx')
        self.detector_mt_tz = dp('i11-ma-cx1/dt/dtc_ccd.1-mt_tz')
        self.obx = dp('i11-ma-c04/ex/obx.1')
        self.mono_mt_rx = dp('i11-ma-c03/op/mono1-mt_rx')
        self.ble = dp('i11-ma-c00/ex/beamlineenergy')
        self.xbpm = dp('i11-ma-c04/dt/xbpm_diode.1')
    ...
    def wait(self, device):
        ...
        # print time.asctime(), str(__name__), 'INFO', 'waiting for device' +
        # str(device)
        ...
        if position != {}:
            for motor in position:
                while abs(self.md2.read_attribute(self.shortFull[motor]).value - position[motor]) > epsilon:
                    self.wait(self.md2)
                    self.md2.write_attribute(
                        self.shortFull[motor], position[motor])
            self.wait(self.md2)
        return
    ...
    def getMotorValues(self):
        ...
        for motor in self.motorShortNames:
            position[motor] = self.md2.read_attribute(
                self.shortFull[motor]).value
        return position
    ...
    def getHelicalCollectPositions(self, helicalSteps):
        start = self.helicalStart
        final = self.helicalFinal
        
        displacements = dict([(motor, final[motor] - start[motor]) for motor in start])
        step = 1./(helicalSteps - 1)
        points = numpy.arange(0., 1. + step, step)
        positions = [dict([(motor, start[motor] + point * displacements[motor]) for motor in start])
                                                                                for k in xrange(self.helicalWedge)
                                                                                for point in points]
        return positions
    ...
    def calculateLinearCollectPosition(self, n):
        ...
        position = {}
        for motor in start:
            position[motor] = start[motor] + \
                displacements[motor] * float(n) / self.nImages
    ...
    def scan(self, center, nbsteps, lengths, attributes):
        ...
        positions = list(
            itertools.product(range(nbsteps[0]), range(nbsteps[1]), [1]))
    ...
    def calculateGridPositions(self, start=[0., 0.], nbsteps=[15, 10], lengths=[1.5, 1.], angle=0., motors=['PhiY', 'PhiZ']):
        ...
        dictionariesOfOrderedPositions = [{motors[0]: position[0], motors[1]: position[1]}
    ...
    def getCollectPositions(self, imageNums):
        ...
        if self.helical:
            if self.nImages < 2:
                raise Exception('With helical scan at least two images have to be requested, please check the input parameters')
                
            helicalSteps = self.nImages / self.helicalWedge
            if self.nImages % self.helicalWedge != 0: helicalSteps += 1

            positions = self.getHelicalCollectPositions(helicalSteps)
     
        elif self.linear:
            positions = [self.calculateLinearCollectPosition(n)
                         for n in range(self.nImages)]
        elif self.grid:
            positions = self.calculateGridPositions(
                self.grid_start, self.grid_nbsteps, self.grid_lengths)
        else:
    ...
    def initializeDetectorAttributes(self):
        self.logger.info(
            'Reading all the attributes of limaadsc and adsc device servers')
        pass
    ...
    def setupHeader(self):
        ...
        head = re.sub('BEAM_CENTER_X=\d\d\d\.\d',
                      'BEAM_CENTER_X=' + BeamCenterX, head)
        head = re.sub('BEAM_CENTER_Y=\d\d\d\.\d',
                      'BEAM_CENTER_Y=' + BeamCenterY, head)
        return head
    ...
    def setEnergy(self, energy=None):
        ...
        if energy is None:
           return self.ble.read_attribute('energy').value
    ...
    def setAttenuation(self, attenuation=None):
        import PyTango
        Attenuator = PyTango.DeviceProxy('i11-ma-c05/ex/att.1')
        labels = ['00 Extract',
                  '01 Carbon 200um',
                  '02 Carbon 250um',
                  '03 Carbon 300um',
    ...
    def prepareWedges(self, firstImage, nbFrames, ScanStartAngle):
        ...
        if self.inverse is None:
            ...
            for k in range(0, numberOfFullWedges):
                _ScanStartAngle = ScanStartAngle + k * \
                    wedgeSize * (self.ScanRange - self.ScanOverlap)
            ...
            _ScanStartAngle = ScanStartAngle + numberOfFullWedges * \
                wedgeSize * (self.ScanRange - self.ScanOverlap)
    ...
    def collect(self):
        ...
        self.prepareWedges(self.firstImage, 
                           self.nbFrames, 
                           self.ScanStartAngle)
    ...
    def collectWedge(self, wedge):
        ...
        for imageNum, position in zip(wedge['imageNumbers'], wedge['positions']):
            ...
            self.lastImage(
                integer=self.imageNum, imagePath=self.imagePath, fileName=fileName)
    ...
    def collectImage(self, fileName, ScanStartAngle):
        ...
        while imageTaken is False and self.stt not in ['STOP', 'ABORT']:
            try:
                self.logger.info(
                    'Collecting image ' + fileName + ' at angle ' + str(ScanStartAngle))
                self.wait(self.adsc)
                ...
                traceback.print_exc()
                self.logger.info(
                    'Problem occured during collection of image ' + fileName)
    ...
    def limaadscStop(self, timeout=0.1):
        ...
        while self.limaadsc.log[-1].find('Acquisition is Stopped.') == -1:
            ...
            if k > 1:
                self.logger.info(
                    'Problem executing Stop command on limaadsc. Attempt %d to stop it.' % k)
            time.sleep(0.05)
        return
...
def main():
    ...
    parser.add_option('-e', '--exposure', default=0.5,
                      type=float, help='exposure time (default: %default)')
    parser.add_option('-o', '--oscillation', default=0.5,
                      type=float, help='oscillation range (default: %default)')
    parser.add_option('-p', '--passes', default=1,
                      type=int, help='number of passes (default: %default)')
    parser.add_option('-s', '--start', default=0.0,
                      type=float, help='collect start angle (default: %default)')
    parser.add_option('-n', '--nImages', default=1, type=int,
                      help='Number of images to collect (default = %default')
    parser.add_option('-r', '--range', default=None, type=float,
                      help='collect range. This is alternative way to specify how much we want to explore (alternative to --nImages)')
    parser.add_option('-a', '--anticipation', default=1,
                      type=int, help='scan anticipation (default: %default)')
    parser.add_option('-l', '--overlap', default=0.0,
                      type=float, help='scanning overlap (default: %default)')
    parser.add_option('-d', '--directory', default='/tmp/test-data',
                      type=str, help='where to store collected images (default: %default)')
    parser.add_option(
        '-u', '--run', default=1, type=int, help='run number')
    parser.add_option('-f', '--firstImage', default=1, type=int,
                      help='Image number to start with. Useful if some images were collected already and we do not want to overwrite them (default: %default)')
    parser.add_option('-x', '--prefix', default='F6',
                      type=str, help='prefix (default = %default)')
    parser.add_option('-i', '--suffix', default='img',
                      type=str, help='suffix (default = %default)')
    parser.add_option('-t', '--template', default='prefix_1_####.img',
                      type=str, help='teplate (default = %default)')
    parser.add_option('-c', '--comment', default='',
                      type=str, help='Add your comment here ...')
    parser.add_option('-I', '--inverse', default=None,
                      help='Inverse collects, parameter is integer specifying reference interval i.e. number of images in the wedge (default = %default)')
    parser.add_option('-E', '--energy', default=None,
                      help='Energy at which to perform the collect')
    parser.add_option('-R', '--resolution', default=None,
                      help='Resolution at which to perform the collect')
    parser.add_option('-D', '--distance', default=None,
                      help='Detector distance at which to perfom the collect')
    parser.add_option('-T', '--transmission', default=None,
                      help='Set transmission for the collect')
    parser.add_option('-A', '--attenuation', default=None,
                      help='Set the attenuation for the collect')
    parser.add_option('-N', '--test', default=False,
                      help='Collect without beam (do not attempt to open the safety shutter')
    ...
    if options.range != None:
        nbFrames = int(
            math.ceil((options.range - options.start) / options.oscillation))
    else:
