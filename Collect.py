...
class collect(object):
    ...
    def beamCenter(self):
        self.logger.info('beamCenter calculation')
        ...
        Origin = Theta.T * X.T
        Origin = Origin * q
        print 'beamCenter', Origin[1] + zcor, Origin[0] + xcor
    ...
    def adxvBeamCenter(self):
        self.logger.info('adxvBeamCenter')
        template = '%s %s 3072 3072'
        q = 0.102592 # ADSC Q315 pixel size
        X, Y = self.beamCenter()
        x, y = int(X/q), int(Y/q)
        print 'X, Y (mm)', X, Y
        print 'x, y (px)', x, y
        ...
        f = open(os.path.join(self.imagePath, '.adxv_beam_center'), 'w')
        f.write(template % (y, x))
        f.close()
...
def main():
    ...
    if options.energy != None:
        options.energy = float(options.energy)
    ...
    collectObject = collect(    
                            ...
                            test = False)
    print 'collectObject', collectObject
    collectObject.collect()
