...
class collect(object):
    ...
    def calculateLinearCollectPosition(self, n):
        start = self.linearStart
        final = self.linearFinal
        displacements = {}
        for motor in start:
    ...
    def getCollectPositions(self, imageNums):
        ...
        if self.helical:
            ...
            positions = self.calculateGridPositions(self.grid_start, self.grid_nbsteps, self.grid_lengths, self.grid_angle)
        else:
    ...
    def safeOpenSafetyShutter(self):
        self.logger.info('Sefe open the safety shutter')
        if self.pss.pssStatusEH == 1:
            self.obx.Open()
